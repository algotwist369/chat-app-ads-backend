...
// Default services & time slots (unchanged)
const DEFAULT_SERVICES = [/* ... your full array remains exactly the same ... */];
const DEFAULT_TIME_SLOTS = [
  { label: "10:00 AM – 12:00 PM", action: "slot_morning" },
  { label: "12:00 PM – 2:00 PM", action: "slot_midday" },
  { label: "2:00 PM – 4:00 PM", action: "slot_afternoon" },
...
const getAutoReplyConfig = async (managerId) => { /* unchanged */ };
const invalidateAutoReplyConfigCache = (managerId) => { /* unchanged */ };
setInterval(() => { /* cleanup */ }, 10 * 60 * 1000);

const getManagerDetails = async (managerId) => { /* unchanged */ };
const formatDate = (date) => { /* unchanged */ };
const getBookingState = (conversation) => { /* unchanged */ };
const saveBookingState = async (conversationId, bookingData) => { /* unchanged */ };

// ──────────────────────────────
//  REFINED PROFESSIONAL MESSAGES
// ──────────────────────────────
...
const getWelcomeMessage = async (_managerName, customerName, managerBusinessName, managerDetails, autoReplyConfig) => {
  ...
  if (autoReplyConfig?.welcomeMessage?.content) {
    ...
    return { content, quickReplies: autoReplyConfig.welcomeMessage.quickReplies || [] };
  }

  return {
    content: `Hello${customerName ? `, ${customerName}` : ""}.\n\nWelcome to ${managerBusinessName}. We are delighted to assist you with a serene and personalised spa experience.\n\nFirst-time guests enjoy 10% off or a complimentary 15-minute neck & shoulder ritual with any full treatment.\n\nHow may we assist you today?`,
    quickReplies: [
      { text: "Book Appointment", action: "book_now" },
      { text: "View Services & Rates", action: "services_pricing" },
      { text: "Claim Welcome Offer", action: "claim_offer" },
      { text: "Contact Concierge", action: "call_spa" },
    ],
  };
};
...
const getBotResponse = async (message, action = null, _messageCount = 0, conversation = null, managerDetails = null, customerName = null, autoReplyConfig = null) => {
  const lowerMessage = (message || "").toLowerCase().trim();
  const services = autoReplyConfig?.services?.length > 0 ? autoReplyConfig.services : DEFAULT_SERVICES;
  const timeSlots = autoReplyConfig?.timeSlots?.length > 0 ? autoReplyConfig.timeSlots : DEFAULT_TIME_SLOTS;
  const bookingState = conversation ? getBookingState(conversation) : null;

  // Talk with Manager
  if (action === "talk_with_manager" || /talk.*manager|human|real person|connect.*manager/i.test(lowerMessage)) {
    const custom = autoReplyConfig?.responses?.talkWithManager;
    if (custom?.content) return { content: custom.content, quickReplies: custom.quickReplies || [], disableAutoChat: true };

    return {
      content: "I will connect you with our manager immediately. They will respond shortly. Thank you for your patience.",
      quickReplies: [],
      disableAutoChat: true,
    };
  }
  ...
  // Claim Offer
  if (action === "claim_offer" || /claim|offer|yes.*offer/i.test(lowerMessage)) {
    const custom = autoReplyConfig?.responses?.claimOffer;
    const serviceList = services.slice(0, 3).map(s => `• ${s.name} – ${s.description}`).join("\n");

    if (custom?.content) {
      return {
        content: custom.content.replace(/\{serviceList\}/g, serviceList),
        quickReplies: custom.quickReplies.length > 0 ? custom.quickReplies : [
          ...services.slice(0, 3).map(s => ({ text: s.name, action: s.action })),
          { text: "View All Treatments", action: "services_pricing" },
          { text: "Contact Concierge", action: "call_spa" },
        ],
        bookingData: { offerClaimed: true },
      };
    ...
    return {
      content: `Thank you. Your welcome offer (10% off or complimentary 15-minute neck ritual) has been applied.\n\nOur most requested treatments:\n${serviceList}\n\nWhich would you like to reserve?`,
      quickReplies: [
        ...services.slice(0, 3).map(s => ({ text: s.name, action: s.action })),
        { text: "View All Treatments", action: "services_pricing" },
        { text: "Contact Concierge", action: "call_spa" },
  ...
  // Services & Pricing
  if (action === "services_pricing" || /service|menu|price|rate/i.test(lowerMessage)) {
    const topServices = services.slice(0, SERVICE_CHUNK_SIZE);
    const hasMore = services.length > SERVICE_CHUNK_SIZE;
    const list = topServices.map(s => `• ${s.name} — ${s.description}`).join("\n") +
      (hasMore ? "\n\n…Tap 'More Treatments' for additional options." : "");

    const custom = autoReplyConfig?.responses?.servicesPricing;
    if (custom?.content) {
      return {
        content: custom.content.replace(/\{serviceList\}/g, list),
        quickReplies: custom.quickReplies.length > 0 ? custom.quickReplies : [
          ...topServices.slice(0, 3).map(s => ({ text: s.name, action: s.action })),
          { text: "Book Appointment", action: "book_now" },
          { text: "Claim Offer", action: "claim_offer" },
          ...(hasMore ? [{ text: "More Treatments", action: "services_more" }] : []),
        ],
        bookingData: { serviceBrowseOffset: SERVICE_CHUNK_SIZE, servicesFullyBrowsed: !hasMore },
      };
    }

    return {
      content: `Signature Treatments:\n\n${list}\n\nAll sessions include a private suite, personalised consultation, and post-treatment relaxation lounge access.\n\nShall we secure your preferred time?`,
      quickReplies: [
        ...topServices.slice(0, 3).map(s => ({ text: s.name, action: s.action })),
        { text: "Book Appointment", action: "book_now" },
        { text: "Claim Offer", action: "claim_offer" },
  ...
  // More Services
  if (action === "services_more") {
    const offset = bookingState?.serviceBrowseOffset ?? SERVICE_CHUNK_SIZE;
    const next = services.slice(offset, offset + SERVICE_CHUNK_SIZE);
    if (next.length === 0) {
      return {
        content: "You have viewed our full menu. Please let us know your preference or desired experience, and we will recommend accordingly.",
        quickReplies: [
          { text: "Book Appointment", action: "book_now" },
          { text: "Claim Offer", action: "claim_offer" },
          { text: "Contact Concierge", action: "call_spa" },
    ...
    const list = next.map(s => `• ${s.name} — ${s.description}`).join("\n");
    const hasMore = services.length > offset + next.length;

    return {
      content: `Additional Treatments:\n\n${list}\n\nPlease select your preferred option and we will arrange the ideal time.`,
      quickReplies: [
        ...next.slice(0, 3).map(s => ({ text: s.name, action: s.action })),
        { text: "Book Appointment", action: "book_now" },
        { text: "Claim Offer", action: "claim_offer" },
        ...(hasMore ? [{ text: "More Treatments", action: "services_more" }] : []),
      ],
      bookingData: { serviceBrowseOffset: offset + next.length, servicesFullyBrowsed: !hasMore },
    };
  }

  // Book Now
  if (action === "book_now" || /book|appointment|schedule/i.test(lowerMessage)) {
    const custom = autoReplyConfig?.responses?.bookNow;
    if (custom?.content) {
      return { 
        content: custom.content, 
        quickReplies: (custom.quickReplies && custom.quickReplies.length > 0) 
          ? custom.quickReplies 
          : [
            ...services.slice(0, 3).map(s => ({ text: s.name, action: s.action })),
            { text: "View All Services", action: "services_pricing" },
            { text: "Contact Concierge", action: "call_spa" },
            { text: "Location", action: "spa_location" },
          ]
      };
    }

    return {
      content: "Certainly. Please select your desired treatment and we will reserve the most suitable time for you.",
      quickReplies: [
        ...services.slice(0, 3).map(s => ({ text: s.name, action: s.action })),
        { text: "View All Services", action: "services_pricing" },
        { text: "Contact Concierge", action: "call_spa" },
  ...
  // Service Selected
  const selectedService = services.find(s => action === s.action || lowerMessage.includes(s.name.toLowerCase()));
  if (selectedService) {
    const custom = autoReplyConfig?.responses?.serviceSelected;
    if (custom?.content) {
      return {
        content: custom.content
          .replace(/\{serviceName\}/g, selectedService.name)
          .replace(/\{serviceDescription\}/g, selectedService.description),
        quickReplies: custom.quickReplies.length > 0 ? custom.quickReplies : [
          ...timeSlots.map(t => ({ text: t.label, action: t.action })),
          { text: "Change Treatment", action: "book_now" },
          { text: "Contact Concierge", action: "call_spa" },
        ],
        bookingData: { service: selectedService.name, serviceDescription: selectedService.description },
      };
    }
  ...
  // Time Slot Selected → Booking Confirmed
  const selectedSlot = timeSlots.find(t => action === t.action || lowerMessage.includes(t.label.toLowerCase()));
  if (selectedSlot) {
    const serviceName = bookingState?.service || "Your selected treatment";
    const offerText = bookingState?.offerClaimed ? " + Welcome Offer Applied" : "";
    const businessName = managerDetails?.businessName || "Our Spa";
    const locationLink = managerDetails?.locationLink || "https://maps.google.com/?q=Spa+Location";
    const customerDisplayName = customerName || "Valued Guest";
    const formattedDate = formatDate(bookingState?.date);

    const custom = autoReplyConfig?.responses?.bookingConfirmed;
    if (custom?.content) {
      return {
        content: custom.content
          .replace(/\{customerName\}/g, customerDisplayName)
          .replace(/\{date\}/g, formattedDate)
          .replace(/\{time\}/g, selectedSlot.label)
          .replace(/\{serviceName\}/g, serviceName)
          .replace(/\{offerText\}/g, offerText)
        ...
        quickReplies: custom.quickReplies.length > 0 ? custom.quickReplies : [
          ...
          { text: "View Location", action: "spa_location" },
          { text: "Contact Concierge", action: "call_spa" },
          { text: "Speak with Manager", action: "talk_with_manager" },
        ],
        bookingData: { ...bookingState, timeSlot: selectedSlot.label, confirmed: true },
      };
    }
  ...
  // Location
  if (action === "spa_location" || /location|address|where/i.test(lowerMessage)) {
    const link = managerDetails?.locationLink || "https://maps.google.com/?q=Spa+Location";
    const name = managerDetails?.businessName || "Our Spa";

    const custom = autoReplyConfig?.responses?.location;
    if (custom?.content) return { content: custom.content.replace(/\{locationLink\}/g, link).replace(/\{businessName\}/g, name), quickReplies: custom.quickReplies || [] };
  ...
  // Call / Contact
  if (action === "call_spa" || /call|phone|contact/i.test(lowerMessage)) {
    const phone = managerDetails?.phone || "+91 9125846358";
    const name = managerDetails?.businessName || "Our Spa";

    const custom = autoReplyConfig?.responses?.callSpa;
    if (custom?.content) return { content: custom.content.replace(/\{phone\}/g, phone).replace(/\{businessName\}/g, name), quickReplies: custom.quickReplies || [] };

    return {
      content: `You may reach us directly at ${phone}.\n\nA team member will be happy to assist you personally.`,
      quickReplies: [
        { text: "Book Appointment", action: "book_now" },
        { text: "View Location", action: "spa_location" },
        { text: "Speak with Manager", action: "talk_with_manager" },
  ...
  if (/thank/i.test(lowerMessage)) {
    const custom = autoReplyConfig?.responses?.thankYou;
    if (custom?.content) return { content: custom.content, quickReplies: custom.quickReplies || [] };

    return {
      content: "You’re most welcome. We remain at your service for your next visit.",
      quickReplies: [
        { text: "Book Appointment", action: "book_now" },
        { text: "Claim Offer", action: "claim_offer" },
        { text: "Contact Concierge", action: "call_spa" },
  ...
  if (/^$|hello|hi|hey/i.test(lowerMessage)) {
    const custom = autoReplyConfig?.responses?.greeting;
    if (custom?.content) return { content: custom.content, quickReplies: custom.quickReplies || [] };

    return {
      content: `Good day. Welcome to ${managerDetails?.businessName || "our spa"}.\n\nHow may we assist you today?`,
      quickReplies: [
        { text: "Claim Welcome Offer", action: "claim_offer" },
        { text: "View Services & Rates", action: "services_pricing" },
        { text: "Book Appointment", action: "book_now" },
  ...
  // Default Fallback
  const custom = autoReplyConfig?.responses?.default;
  if (custom?.content) return { content: custom.content.replace(/\{message\}/g, message || ""), quickReplies: custom.quickReplies || [] };

  return {
    content: "Thank you for your message.\n\nI am here to assist with treatment selection, pricing, reservations, or to connect you with our manager.\n\nPlease select an option below:",
    quickReplies: [
      { text: "Claim Offer", action: "claim_offer" },
      { text: "View Services", action: "services_pricing" },
      { text: "Book Now", action: "book_now" },
      { text: "Contact Concierge", action: "call_spa" },
    ],
  };
...
// ──────────────────────────────
const sendWelcomeMessage = async (conversationId, managerId, managerName, customerName, managerBusinessName) => { /* unchanged logic */ };
const processCustomerMessage = async (conversationId, customerMessage, action = null) => { /* unchanged logic */ };
const disableAutoChat = async (conversationId) => { /* unchanged */ };
